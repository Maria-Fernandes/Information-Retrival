...
public class Test{
        public static class InvertedIndexBigrams extends Mapper<LongWritable, Text, Text, Text>
        {
                ...
                public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
                {
                   ...
                   for (int i = 0; i < words.length - 2 + 1; i++){
                      keyword.set(concatination(words, i, i+2));
                      context.write(keyword, documentId);
                   }
                }
        }

        public static String concatination(String[] biwords, int initialindex, int finalindex) {
        StringBuilder builder = new StringBuilder();
        for (int i = initialindex; i < finalindex; i++)
            builder.append((i > initialindex ? " " : "") + biwords[i]);
